...
function Login(props)
{
    ...
    async function fetchData(data) {
        ...
        // if the user fetched successfully, set all the states in App.js
        if(responseMessage.status === 200) {
            props.setUser(userData[0].username);
            props.setEmail(userData[0].email);
            props.setName(userData[0].name);
            props.setAddress(userData[0].address);
            props.setId(userData[0].id);
            // redirect to /app
    ...
    return (
        <div id="login_window">
            ...
            <NavLink className="link" to="/create_account"><span id="create_account">Dont't have an account? Sign Up</span></NavLink>
        </div>
    );
}
